...
export const SalesDetailModal: React.FC<SalesDetailModalProps> = ({ sale, onClose }) => {
  ...
  const receiptData: SalesReceiptData = {
    ...
    affiliate_name: sale.affiliates?.full_name || null,
        ...
        <div className="space-y-4 text-left">
          ...
          <div className="space-y-1 pt-1">
            <div className="flex items-center justify-between py-2 border-b border-slate-100 text-xs">
              ...
              <span className="px-2.5 py-0.5 rounded-lg bg-slate-100 text-slate-800 font-bold border border-slate-200/80">
            ...
            {sale.products?.weight && (
              <div className="flex items-center justify-between py-2 border-b border-slate-100 text-xs">
                <span className="text-slate-500 font-semibold">Berat Bersih</span>
                <span className="px-2.5 py-0.5 rounded-lg bg-slate-100 text-slate-800 font-bold border border-slate-200/80">
                  {sale.products.weight}
                </span>
              </div>
            ...
            <div className="flex items-center justify-between py-2 border-b border-slate-100 text-xs">
              <span className="text-slate-500 font-semibold">Kode Referral</span>
              {hasReferral ? (
                <span className="px-2.5 py-0.5 rounded-lg bg-purple-50 text-purple-700 font-mono font-black text-xs border border-purple-200/80">
                  {sale.referral_code}
                </span>
              ) : (
                <span className="px-2.5 py-0.5 rounded-lg bg-slate-100 text-slate-500 font-semibold text-xs border border-slate-200/80">
                  Tanpa Referral (Direct)
                </span>
              )}
            </div>

            {hasReferral && sale.affiliates?.full_name && (
              <div className="flex items-center justify-between py-2 border-b border-slate-100 text-xs">
                <span className="text-slate-500 font-semibold">Nama Mitra</span>
                <span className="px-2.5 py-0.5 rounded-lg bg-sky-50 text-sky-800 font-bold text-xs border border-sky-200/80 capitalize">
                  {sale.affiliates.full_name}
                </span>
              </div>
            )}

            {hasReferral && sale.affiliates?.email && (
              <div className="flex items-center justify-between py-2 border-b border-slate-100 text-xs">
                <span className="text-slate-500 font-semibold">Email Mitra</span>
                <span className="px-2.5 py-0.5 rounded-lg bg-slate-100 text-slate-600 font-medium text-xs border border-slate-200/80">
                  {sale.affiliates.email}
                </span>
              </div>
            )}

            <div className="flex items-center justify-between py-2 text-xs">
              <span className="text-slate-500 font-semibold">ID Transaksi</span>
              <span className="font-mono text-[11px] text-slate-500 bg-slate-100 px-2 py-0.5 rounded-lg border border-slate-200/80 truncate max-w-[180px] sm:max-w-[220px]">
                {sale.id}
              </span>
            </div>
          </div>
