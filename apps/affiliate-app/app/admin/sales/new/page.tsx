...
export default function AdminNewSalePage() {
  ...
  const commissionPreview = selectedAffiliate || referralCodeInput ? reqQty * 2000 : 0;

  // Filter affiliates for autocomplete
  ...
  const handleSelectAffiliate = (aff: any) => {
    ...
    setReferralCodeInput(aff.referral_code);
    ...
    try {
      ...
      if (!res.ok || !data.success) {
        ...
        if (
          ...
          errMessage.includes("sales_affiliate_id_fkey") ||
          errMessage.includes("foreign key")
        ) {
          errMessage = "Kode Referral tidak terdaftar";
        }
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans pb-12">
          ...
          <main className="flex-1 min-w-0 w-full max-w-2xl mx-auto lg:mx-0 space-y-6 text-left">
            <div className="px-1 space-y-1.5">
              ...
              <p className="text-sm font-medium text-slate-500 leading-relaxed">
                Masukkan detail produk & Kode Referral yang digunakan konsumen saat membeli.
              </p>
            </div>
            ...
            <form onSubmit={handleFormSubmit} className="space-y-5">
              ...
              <div className="grid grid-cols-3 gap-2.5 sm:gap-3">
                ...
                <div className="col-span-2 space-y-1 relative">
                  ...
                  <div className="relative">
                    ...
                    <input
                      type="text"
                      placeholder="Ketik nama / kode referral..."
                  ...
                  {showAutocomplete && referralCodeInput && filteredAffiliates.length > 0 && (
                    <div className="absolute top-full left-0 right-0 mt-1 bg-white rounded-2xl border border-slate-200 shadow-xl max-h-48 overflow-y-auto z-20 divide-y divide-slate-100">
                      {filteredAffiliates.map((aff) => (
                        ...
                        >
                          <div>
                            <p className="text-xs font-extrabold text-slate-900">{aff.full_name}</p>
                            <p className="text-[11px] font-mono font-bold text-dophy-600">{aff.referral_code}</p>
                          </div>
                          <span className="text-[10px] font-bold text-slate-400">{aff.email}</span>
                        </button>
                      ))}
                    </div>
                  )}
              ...
              <div className="space-y-2.5">
                ...
                <div className="p-3.5 sm:p-4 rounded-2xl bg-gradient-to-r from-sky-500/15 via-blue-500/10 to-indigo-500/15 border border-sky-200/90 shadow-2xs flex items-center justify-between gap-3 text-left relative overflow-hidden group">
                  ...
                  <div className="relative z-10 min-w-0 space-y-0.5">
                    <span className="text-xs font-black text-slate-900 block truncate">Komisi Mitra Affiliator</span>
                    <span
                      className={`text-[11px] font-semibold block truncate ${
                        commissionPreview > 0 ? "text-sky-800 font-bold" : "text-slate-500"
                      }`}
                    >
                      {commissionPreview > 0 ? "Bonus Referral Rp 2.000 / pcs" : "Tanpa kode referral (Rp 0)"}
                    </span>
                  </div>
      ...
      <CustomConfirmModal
        ...
        variant="warning"
        title="Tanpa Kode Referral?"
        description="Transaksi penjualan ini akan dicatat tanpa menggunakan kode referral affiliator. Tidak ada alokasi saldo komisi yang dihitung. Apakah Anda ingin melanjutkan simpan?"
