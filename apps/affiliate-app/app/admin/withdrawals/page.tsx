...
import { uploadAffiliateAssetToCloudinary } from "@repo/cloudinary";
...
export default function AdminWithdrawalsPage() {
  ...
  const fetchWithdrawals = async (statusFilter: string) => {
    setIsLoading(true);
    try {
  ...
  const handleApproveSubmit = async () => {
    ...
    try {
      const affiliate = selectedApproveWd.affiliates || {};
      const referralCode = affiliate.referral_code || "DOPHY-GENERAL";
      const fullName = affiliate.full_name || "Affiliate";

      // 1. Upload proof screenshot to Cloudinary subfolder:
      // dophy/{env}/affiliators/{referralCode}_{slugName}/withdrawals/
      const uploadRes = await uploadAffiliateAssetToCloudinary({
        file: proofFile,
        referralCode,
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans pb-12">
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-30 shadow-2xs">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <h1 className="text-base font-black text-slate-900 tracking-tight">Approval Penarikan Dana</h1>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 pt-6">
        <div className="flex flex-col lg:flex-row gap-6 items-start">
          ...
          <main className="flex-1 min-w-0 w-full space-y-5">
            {/* Status Filter Chips (Flexible Grid - No Horizontal Scrollbar) */}
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              {[
                ...
                <button
                  ...
                  className={`w-full py-2.5 px-3 rounded-2xl text-xs font-extrabold transition-all cursor-pointer flex items-center justify-center text-center shadow-2xs ${
            ...
            {isLoading ? (
              ...
              <div className="bg-white rounded-3xl p-8 border border-slate-200 text-center space-y-2">
                ...
                <p className="text-xs text-slate-500 font-medium max-w-xs mx-auto">
                  Seluruh pengajuan penarikan dana dari affiliator dengan status filter ini telah bersih.
                </p>
              </div>
            ) : (
              <div className="space-y-2.5">
                {withdrawals.map((item, index) => {
                  const affiliate = item.affiliates || {};
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between gap-2">
                        <h3 className="text-sm sm:text-base font-black text-slate-900 group-hover:text-dophy-600 transition-colors truncate">
                          {affiliate.full_name || "Affiliator"}
                        </h3>
                      ...
                      <div className="flex items-center gap-1.5 flex-wrap">
                        <span className="px-2 py-0.5 rounded-md bg-purple-50 text-purple-700 font-mono text-[10.5px] font-extrabold border border-purple-200/80 shadow-2xs shrink-0">
                          {affiliate.referral_code || "TANPA KODE"}
                        </span>
                        {affiliate.email && (
                          <span className="px-2 py-0.5 rounded-md bg-slate-100 text-slate-600 font-bold text-[10.5px] border border-slate-200/80 shadow-2xs truncate">
                            {affiliate.email}
                          </span>
                        )}
                      </div>
      ...
      <ResponsiveDetailModal
        ...
        subtitle={`Affiliator: ${selectedApproveWd?.affiliates?.full_name} (${selectedApproveWd?.affiliates?.referral_code})`}
        footer={
      ...
      <ResponsiveDetailModal
        ...
        subtitle={`Saldo Rp ${Number(selectedRejectWd?.amount || 0).toLocaleString("id-ID")} akan dikembalikan ke affiliator`}
        footer={
          <div className="flex items-center justify-end gap-2.5 w-full">
            <button
              type="button"
      ...
      <ResponsiveDetailModal
        ...
        subtitle={`Affiliator: ${selectedDetailWd?.affiliates?.full_name || "Affiliator"}`}
        footer={
          selectedDetailWd ? (
          ...
          <div className="space-y-3.5 text-left">
            ...
            <div className="p-3.5 rounded-2xl bg-slate-50/90 border border-slate-200/80 space-y-2">
              <div className="flex items-center justify-between gap-2">
                ...
                <StatusBadge status={selectedDetailWd.status} />
              </div>
            </div>
            ...
            <div className="p-3.5 rounded-2xl bg-slate-50/90 border border-slate-200/80 space-y-2">
              ...
              <div className="space-y-1.5 text-xs">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-extrabold text-slate-900">{selectedDetailWd.affiliates?.full_name || "-"}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-slate-500 font-medium">Kode Referral:</span>
                  <span className="px-2 py-0.5 rounded-md bg-purple-50 text-purple-700 font-mono text-[10.5px] font-extrabold border border-purple-200/80">
                    {selectedDetailWd.affiliates?.referral_code || "TANPA KODE"}
                  </span>
                </div>
                {selectedDetailWd.affiliates?.email && (
                  <div className="flex items-center justify-between">
                    <span className="text-slate-500 font-medium">Email:</span>
                    <span className="font-bold text-slate-700">{selectedDetailWd.affiliates.email}</span>
                  </div>
                )}
              </div>
            </div>
                ...
                {selectedDetailWd.completed_at && (
                  <div className="flex items-center justify-between gap-2 flex-wrap pt-1.5 border-t border-slate-200/60">
                    <span className="text-slate-500 font-bold shrink-0">
                      {selectedDetailWd.status === "rejected" ? "Ditolak:" : "Diselesaikan:"}
                    </span>
                    <span className="font-extrabold text-slate-800 text-right">
            ...
            {selectedDetailWd.status === "rejected" && selectedDetailWd.rejection_reason && (
