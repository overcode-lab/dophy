...
export default function AffiliateWithdrawPage() {
  ...
  useEffect(() => {
    ...
    fetchAffiliateProfile(parsedUser.id);
  }, [router]);

  const fetchAffiliateProfile = async (id: string) => {
    setIsLoading(true);
    try {
      const res = await fetch(`/api/affiliate/dashboard?affiliate_id=${id}`);
      const data = await res.json();
      if (data.success) {
        setAffiliateData(data.data.affiliate);
        setBankName(data.data.affiliate.bank_name || "");
        setAccountNumber(data.data.affiliate.bank_account_number || "");
      }
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const availableBalance = Number(affiliateData?.available_balance || user?.available_balance || 0);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/affiliate/withdrawals", {
        ...
        body: JSON.stringify({
          affiliate_id: affiliateData?.id || user?.id,
    ...
    <div className="min-h-screen bg-slate-50 font-sans pb-12">
      ...
      <header className="bg-white border-b border-slate-100 sticky top-0 z-30">
        <div className="max-w-md mx-auto px-4 py-3 flex items-center gap-2.5">
          ...
          <h1 className="text-base font-black text-slate-900 tracking-tight">Penarikan Dana</h1>
        </div>
      </header>
      ...
      <main className="max-w-md mx-auto px-4 pt-5 space-y-5">
        ...
        <div className="space-y-1 text-left px-1">
          <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Saldo Tersedia Saat Ini</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 text-left">
          <div className="space-y-1">
            <label className="text-xs font-bold text-slate-700 block">Nominal Penarikan (Rp)</label>
          ...
          <div className="space-y-3 pt-2">
            ...
            <div className="space-y-1">
              ...
              />
              {errors.accountNumber && <p className="text-[11px] font-semibold text-rose-500">{errors.accountNumber}</p>}
            </div>
          </div>
