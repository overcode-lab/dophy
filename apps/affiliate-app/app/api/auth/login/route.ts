...
export async function POST(request: Request) {
  try {
    ...
    if (role === "admin") {
      ...
      if (!affiliateProfile) {
        return NextResponse.json(
          { success: false, error: "Akun ini belum terdaftar sebagai Affiliator DOPHY." },
          { status: 403 }
        );
      }

      if (affiliateProfile.status === "inactive") {
        return NextResponse.json(
          { success: false, error: "Akun Affiliator Anda sedang dinonaktifkan oleh Admin." },
          { status: 403 }
        );
      }
      ...
      if (!affiliateProfile.user_id) {
        await supabaseAdmin.from("affiliates").update({ user_id: userId }).eq("id", affiliateProfile.id);
      }

      return NextResponse.json({
        success: true,
        role: "affiliate",
        user: { ...affiliateProfile, user_id: userId },
        session: authData.session,
      });
    }
  } catch (error: any) {
