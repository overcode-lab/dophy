...
import { PartnerRegisterSchema, getSupabaseAdminClient } from "@repo/db";
...
export async function POST(request: Request) {
  try {
    ...
    const validationResult = PartnerRegisterSchema.safeParse(body);
    ...
    // 2. Check if partner email already exists in public.partners
    ...
    if (existingPartner) {
      return NextResponse.json(
        {
          success: false,
          error: "Email sudah terdaftar sebagai Creator Partner. Silakan login.",
        },
        { status: 409 }
      );
    }
    ...
    // 5. Insert into public.partners table
    ...
    if (insertError) {
      return NextResponse.json(
        {
          success: false,
          error: `Gagal menyimpan data Creator Partner: ${insertError.message}`,
        },
        { status: 500 }
      );
    }
    ...
    return NextResponse.json(
      {
        success: true,
        message: "Pendaftaran Creator Partner berhasil!",
        data: newPartner,
      },
      { status: 201 }
    );
