  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok || !data.success) {
        setGeneralError(data.error || "Login gagal. Periksa email & password Anda.");
        setIsLoading(false);
        return;
      }

      // Store basic session / user info locally
      if (typeof window !== "undefined") {
        localStorage.setItem("dophy_user", JSON.stringify(data.user));
        localStorage.setItem("dophy_role", "partner");
      }
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4 sm:p-6 font-sans">
      ...
      <div className="fixed inset-0 pointer-events-none overflow-hidden">
        <div className="absolute -top-40 -left-40 w-96 h-96 bg-dophy-100 rounded-full blur-3xl opacity-60" />
        <div className="absolute -bottom-40 -right-40 w-96 h-96 bg-amber-100 rounded-full blur-3xl opacity-60" />
      </div>
      ...
      >
        ...
        <div className="text-left space-y-3">
          ...
          <h1 className="text-2xl sm:text-3xl font-black tracking-tight text-slate-900">Masuk Creator Partner</h1>
          <p className="text-slate-500 text-xs sm:text-sm font-medium leading-relaxed max-w-sm">
            Pantau Creator Royalty real-time, performa Creator Code, dan kelola penarikan dana.
          </p>
        </div>
        ...
        {generalError && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            className="p-3.5 rounded-2xl bg-rose-50 border border-rose-200 text-rose-700 text-xs font-semibold flex items-center gap-2.5 text-left"
          >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-1 text-left">
            <label className="text-xs font-bold text-slate-700 block">Email Partner</label>
            <div className="relative">
              ...
              <input
                ...
                className={`w-full pl-10 pr-4 py-3 sm:py-2.5 rounded-xl border text-sm font-medium transition-all bg-white sm:bg-slate-50/50 focus:bg-white outline-none ${
                  errors.email
          ...
          <div className="space-y-1 text-left">
            ...
            <div className="relative">
              ...
              <input
                ...
                className={`w-full pl-10 pr-10 py-3 sm:py-2.5 rounded-xl border text-sm font-medium transition-all bg-white sm:bg-slate-50/50 focus:bg-white outline-none ${
                  errors.password
                    ? "border-rose-400 focus:border-rose-500 focus:ring-2 focus:ring-rose-100"
                    : "border-slate-200 focus:border-dophy-500 focus:ring-2 focus:ring-dophy-100"
                }`}
        ...
        <div className="pt-4 border-t border-slate-200/80 text-left">
          <p className="text-xs text-slate-500 font-medium">
            ...
            >
              Daftar Creator Partner di sini
            </Link>
          </p>
        </div>
      </motion.div>
