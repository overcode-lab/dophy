-- ====================================================================
-- DOPHY PARTNER APPLICATION - DUMMY SEED DATA SCRIPT
-- ====================================================================
-- Description: Independent SQL script to populate test dummy data for
--              Products, Admin, Partners (Budi & Siti), and Announcement.
...
-- 3. Initial Partner Accounts (Budi & Siti) (Auto-links to auth.users if registered in Supabase Auth)
INSERT INTO public.partners (id, user_id, full_name, email, phone_number, referral_code, bank_account_number, bank_name, status, available_balance, held_balance, sales_target)
VALUES 
    (
...
ON CONFLICT (email) DO UPDATE
SET user_id = COALESCE(public.partners.user_id, (SELECT id FROM auth.users WHERE email = EXCLUDED.email LIMIT 1));
...
VALUES 
    (
        ...
        'Dapatkan Creator Royalty ekstra setiap penjualan snack DOPHY dengan bagikan Creator Code unik milikmu!',
